...
//     // Track viewport: scroll-lock behavior is desktop (lg) only
//     useEffect(() => {
...
//     // Lock page scroll & cycle through items on scroll — DESKTOP ONLY.
//     // Mobile relies purely on button taps (handleTabChange).
//     useEffect(() => {
//         if (!isDesktop) return;
...
//             id={sectionId}
//             ref={containerRef}
//             style={isDesktop ? { height: `${services.length * 100}vh` } : undefined}
//             className={`${bgClass} relative`}
//         >
//             {/* Sticky screen-pin layer only kicks in on desktop; mobile is a normal flow section */}
//             <div
//                 className={`${
//                     isDesktop ? "sticky top-0 h-screen" : "relative"
...
//                     {/* Mobile / Tablet Horizontal Navigation Header — pure button selection, no scroll-driven switching */}
...
//                         {/* Dashboard Navigation Sidebar (Left Sticky Container) — desktop only, hover still cycles on desktop */}
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`w-full text-left px-5 py-4 rounded-xl transition-all duration-300 group relative flex items-center gap-4 ${
                                                isActive
                                                    ? "bg-white text-sky-600 shadow-md shadow-sky-500/10 border-l-4 border-sky-500"
                                                    : "text-slate-600 hover:bg-white/60 hover:text-sky-500 border-l-4 border-transparent"
                                            }`}
                                        >
                                            {/* Number Badge */}
                                            {/* <div
                                                className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold transition-colors duration-300 shrink-0 ${
                                                    isActive
                                                        ...
                                                        : "bg-slate-200/70 text-slate-500 group-hover:bg-sky-100 group-hover:text-sky-600"
                                                }`}
                                            >
                                                {String(idx + 1).padStart(2, "0")}
                                            </div> */}
