  ...
  return (
    <nav className="h-full flex flex-col bg-white font-sans">
          ...
          return (
            <div key={section.key} className="mb-2">
              ...
              >
                ...
                <ChevronDown className={cn('h-3.5 w-3.5 transition-transform duration-300 text-sky-500', open[section.key] ? 'rotate-180' : '')} />
              </button>
                  ...
                  {section.items.map((item) => {
                    ...
                    return (
                      <Link key={item.href} href={item.href} className={cn(
                        ...
                        active ? 'bg-sky-50 text-sky-700 border-l-sky-600 shadow-sm' : 'text-slate-700 hover:bg-sky-50 border-l-transparent hover:border-l-sky-300 hover:text-sky-800'
                      )}>
                        <Icon className={cn('h-4 w-4 transition-colors', active ? 'text-sky-600' : 'text-slate-500')} />
      ...
      <div className="border-t border-slate-200 p-4 bg-sky-white">
        <button onClick={() => signOut({ callbackUrl: '/login' })} className="w-full flex items-center gap-3 px-4 py-3 text-sm text-slate-700 hover:bg-white hover:text-red-600 rounded-lg border border-transparent hover:border-red-200 transition-all duration-200 font-medium">
          <LogOut className="h-4 w-4" />
          <span>Sign Out</span>
        </button>
      </div>

      <style>{`.custom-scrollbar::-webkit-scrollbar{width:4px;}.custom-scrollbar::-webkit-scrollbar-track{background:#f1f5f9;}.custom-scrollbar::-webkit-scrollbar-thumb{background:#bae6fd;border-radius:10px;}.custom-scrollbar::-webkit-scrollbar-thumb:hover{background:#7dd3fc;}`}</style>
    </nav>
  );
}
