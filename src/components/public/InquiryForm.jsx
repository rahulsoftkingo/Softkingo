          ...
          {isDigital ? (
            <div className="h-12 border border-slate-300 rounded-lg bg-white px-3 relative focus-within:border-sky-500 focus-within:ring-1 focus-within:ring-sky-200 transition-all">
              ...
              <div className="flex items-center gap-2 relative -top-4">
                <FaLink className="text-slate-500 shrink-0" size={14} />
                <input
                  id="websiteLink"
                  name="websiteLink"
                  type="url"
                  placeholder={websiteLinkPlaceholder}
                  value={websiteLink}
                  onChange={(e) => setWebsiteLink(e.target.value)}
                  className="w-full border-none outline-none bg-transparent text-sm text-slate-800 placeholder:text-slate-400"
                />
              </div>
            </div>
          ) : (
