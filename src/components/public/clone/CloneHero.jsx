...
export default function CloneHero({ data }) {
  ...
  return (
    <section className="relative min-h-screen h-full py-4 md:py-6 bg-[#1A1A1A] overflow-clip flex flex-col items-center justify-center select-none">
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-6 text-center flex flex-col items-center">
        ...
        <div className="max-w-5xl mx-auto mb-4 animate-in fade-in slide-in-from-top duration-1000">
          <h1 className="text-2xl md:text-3xl lg:text-4xl font-extrabold mb-4 leading-[1.2] tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-white via-sky-100 to-white/80">
            {data?.title || "Clone App Development Company"}
          </h1>
          <div
            className="text-slate-300 text-sm md:text-base leading-[1.8] max-w-3xl mx-auto font-normal opacity-90 prose prose-invert prose-sm"
            dangerouslySetInnerHTML={{ __html: data?.subtitle || "Softkingo has the best Front end developers capable of providing the most intuitive user interfaces and interactive user experience for your project's dream architecture." }}
          />
        </div>
        ...
        <div className="flex justify-center mb-6 md:mb-8">
          <Link href={data?.buttonHref || "/contact"}>
            ...
            >
              {data?.buttonLabel || "Request A Quote"}
            </motion.button>
          </Link>
        </div>
        ...
        <div className="max-w-4xl mx-auto w-full animate-in fade-in slide-in-from-bottom duration-1000 delay-500">
          <motion.div
            className="bg-white rounded-xl md:rounded-2xl p-3 md:p-5 shadow-xl flex flex-col md:flex-row items-center gap-4 md:gap-10 relative overflow-hidden"
          >
