...
export default function CloneHero({ data }) {
  ...
  return (
    <section className="relative min-h-screen h-full py-4 md:py-6 bg-[#1A1A1A] overflow-hidden flex flex-col items-center justify-center select-none w-full">
      {/* Background Banner Image with Overlap/Overlay */}
      {data?.image && (
        <div className="absolute inset-0 z-0">
          <Image
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-6 text-center flex flex-col items-center">
        ...
        <motion.div
          ...
          onMouseEnter={() => setIsPaused(true)}
          onMouseLeave={() => setIsPaused(false)}
          // Yahan 'touch-pan-y' add kiya hai taaki mobile par vertical scroll intact rahe aur screen hile nahi
          className="relative h-[300px] md:h-[380px] w-full max-w-6xl mx-auto mb-8 md:mb-10 flex items-center justify-center perspective-[1200px] cursor-grab active:cursor-grabbing touch-pan-y"
        >
          <div className="relative w-full h-full flex items-center justify-center pointer-events-none">
            <AnimatePresence mode='popLayout'>
              {sliderImages.map((src, i) => {
