                ...
                >
                  ...
                  >
                    <motion.div 
                      ...
                      className={`p-8 md:p-10 bg-white rounded-[2rem] border border-slate-100 shadow-[0_20px_50px_rgba(0,0,0,0.03)] hover:shadow-[0_30px_60px_rgba(0,0,0,0.06)] hover:border-sky-200 transition-all duration-500 group relative overflow-hidden ${
                        isEven ? "md:text-right" : "md:text-left"
                      }`}
                    >
                      {/* Subtle background number */}
                      <span className="absolute -top-4 -right-2 text-8xl font-black text-slate-50 opacity-[0.03] group-hover:opacity-[0.07] transition-opacity pointer-events-none">
                        {i + 1}
                      </span>

                      <div className={`flex items-center gap-5 mb-6 ${isEven ? "md:flex-row-reverse" : ""}`}>
                        <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-sky-50 to-blue-50 text-sky-600 flex items-center justify-center text-xl font-black shadow-inner group-hover:from-sky-600 group-hover:to-blue-700 group-hover:text-white group-hover:rotate-6 transition-all duration-500">
                          {String(i + 1).padStart(2, '0')}
                        </div>
                        <h4 className="text-2xl font-black text-slate-800 tracking-tight group-hover:text-sky-600 transition-colors">
                          {step.title}
                        </h4>
                      </div>
                      ...
                      <div className={`absolute bottom-0 h-1 bg-gradient-to-r from-sky-500 to-blue-600 transition-all duration-500 ${
                        isEven ? "right-0 w-0 group-hover:w-1/2" : "left-0 w-0 group-hover:w-1/2"
                      }`} />
                    </motion.div>
                  </motion.div>
