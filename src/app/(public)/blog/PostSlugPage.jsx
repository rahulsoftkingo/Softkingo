    ...
    if (node.type === "heading") {
      ...
      if (level === 3) {
        ...
        currentSection.blocks.push({
          type: "h3",
          text:
            text || `Subheading ${sectionIndex}.${subIndex}`,
        });
        continue;
      }
    }
    ...
    return [
      {
        id: "content",
        title: fallbackTitle || "Overview",
        blocks: fallbackExcerpt
          ? [{ type: "p", text: fallbackExcerpt }]
          : [],
      },
    ];
  }
...
export default async function PostSlugPage({ sectionKey, slug }) {
  ...
  const post = await prisma.blogPost.findFirst({
    where: {
      slug,
      status: "published",
      ...(config.types?.length ? { type: { in: config.types } } : {}),
    },
    include: {
      category: true,
      author: true,
  ...
  const sections = mapTipTapToSections(
    post.contentJson,
    post.title,
    post.excerpt || ""
  );
  ...
  const fullPost = {
    ...
    thumbnail:
      post.thumbnail || "/images/insights/launch-thumb.png",
  ...
  const relatedNormalized = related.map((p) => ({
    ...
    thumbnail:
      p.thumbnail || "/images/insights/launch-thumb.png",
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4 pb-9 sm:pt-5 sm:pb-12">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1.75fr)_minmax(0,1.1fr)] gap-6 lg:gap-10 items-center">
            <div className="space-y-3 sm:space-y-4">
              ...
              <form action={config.slugBase} className="pt-2 max-w-md">
