...
export default async function SectionPage({ sectionKey, searchParams }) {
  ...
  const normalizedFeatured =
    featured && normalizedPosts.find((p) => p.slug === featured.slug);
    ...
    <div className="min-h-screen bg-slate-50">
      <header className="relative border-b border-slate-200">
        <div className="absolute inset-0 overflow-hidden">
         <Image
  src={config.heroBg || "/images/career/r.png"}
  alt={config.title}
  fill
  priority
...
/>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-10 sm:pt-8 sm:pb-14 text-slate-50">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1.5fr)_minmax(0,1.2fr)] gap-8 items-start">
            ...
            {normalizedFeatured && (
              <Link
                href={`${config.slugBase}/${normalizedFeatured.slug}`}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-12 lg:py-14">
          ...
          <section className="mt-10 sm:mt-12">
            <NewsletterStrip
              ...
              listSlug={
                config.newsletterList?.slug || "blog-newsletter"
              }
              listName={
                config.newsletterList?.name || "Blog Newsletter"
              }
              listDescription={
                config.newsletterList?.description ||
