...
const UserGuide = ({ data }) => {
    ...
    const activeIdx = toc.findIndex(t => t.id === activeTab);
    const tocProgressPct = toc.length > 1 ? (Math.max(activeIdx, 0) / (toc.length - 1)) * 100 : 0;

    if (isRich && blocks.length === 0) {
        return (
            <section className="bg-white py-8 md:py-16 px-6">
                <div className="max-w-4xl mx-auto text-center py-20 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                    <p className="text-slate-400 italic">No guide content has been added yet.</p>
                </div>
            </section>
        );
    ...
    return (
        <section className="bg-white relative overflow-hidden" id="user-guide">
            <motion.div
                className="fixed top-0 left-0 right-0 h-1 bg-gradient-to-r from-sky-400 to-sky-600 origin-left z-[100]"
                style={{ scaleX }}
            />
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 md:py-16 relative">
                ...
                <button
                    onClick={skipGuide}
                    className="hidden lg:flex absolute top-4 right-6 z-20 px-6 py-2 rounded-full border border-slate-200 text-slate-400 text-[11px] font-bold uppercase tracking-widest hover:bg-sky-50 hover:text-sky-600 hover:border-sky-200 transition-all duration-300 items-center gap-2 group"
                >
                    Skip Guide
                    <svg className="w-3 h-3 group-hover:translate-y-0.5 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M19 14l-7 7m0 0l-7-7m7 7V3" />
                ...
                {/* Eyebrow label, consistent with the rest of the site's section headers */}
                <div className="flex justify-center mb-3">
                    <span className="inline-flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.2em] text-sky-600">
                        <span className="w-4 h-[1px] bg-sky-300" />
                        User Guide
                        <span className="w-4 h-[1px] bg-sky-300" />
                    </span>
                </div>
                ...
                {/* Mobile Sticky ToC Bar */}
                <div className="lg:hidden sticky top-4 z-40 mt-8 mb-4">
                    <div className="bg-white/95 backdrop-blur-xl border border-slate-100 rounded-2xl p-4 shadow-lg shadow-slate-200/50 flex items-center justify-between">
                        <button
                            onClick={() => setIsTocOpen(true)}
                            ...
                            <span>
                                ...
                                {toc.length > 0 && (
                                    <span className="ml-2 text-slate-400 font-medium">
                                        {Math.max(activeIdx, 0) + 1}/{toc.length}
                                    </span>
                                )}
                            </span>
                        </button>
                        <button
                            onClick={skipGuide}
                            className="text-[10px] font-bold uppercase tracking-widest text-slate-400 hover:text-sky-500 flex items-center gap-1"
                        >
                            Skip
                            <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M19 14l-7 7m0 0l-7-7m7 7V3" />
                ...
                {isTocOpen && (
                    ...
                    >
                        ...
                        >
                            <div className="flex items-center justify-between mb-8">
                                <h4 className="text-[10px] font-bold uppercase tracking-widest text-sky-600">On this page</h4>
                                <button onClick={() => setIsTocOpen(false)} className="w-8 h-8 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 hover:bg-sky-50 hover:text-sky-500 transition-colors">
                                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M6 18L18 6M6 6l12 12" />
                                    </svg>
                                </button>
                            ...
                            <nav className="flex-1 overflow-y-auto space-y-1 custom-scrollbar">
                                {toc.map((item, i) => (
                                    <button
                                        key={item.id}
                                        onClick={() => scrollToId(item.id)}
                                        className={`w-full text-left flex items-center gap-3 p-3 rounded-xl transition-all ${activeTab === item.id ? 'bg-sky-50 text-sky-600 font-bold' : 'text-slate-500 font-medium hover:bg-slate-50'}`}
                                    >
                                        <span className={`text-[10px] font-bold tabular-nums ${activeTab === item.id ? 'text-sky-500' : 'text-slate-300'}`}>
                ...
                <div className="mt-8 lg:mt-12 flex flex-col lg:flex-row gap-8 lg:gap-16">
                    ...
                    <aside className="hidden lg:block lg:w-1/3 xl:w-1/4">
                        <div className="sticky top-32 h-fit">
                            <div className="bg-white rounded-sm p-8 border border-slate-100 shadow-[0_20px_50px_rgba(15,23,42,0.05)] relative overflow-hidden h-full max-h-[calc(100vh-180px)] flex flex-col">
                                <div className="absolute top-0 right-0 w-32 h-32 bg-sky-50 rounded-full blur-3xl -mr-16 -mt-16 opacity-60" />

                                <div className="relative z-10 flex flex-col h-full overflow-hidden">
                                    <h4 className="text-[10px] font-bold uppercase tracking-widest text-sky-600 mb-6 flex items-center gap-2 shrink-0">
                                        <span className="w-4 h-[1px] bg-sky-300" />
                                        On this page
                                    </h4>
                                    <nav className="space-y-1 relative overflow-y-auto pr-2 custom-scrollbar">
                                        {toc.map((item, i) => (
                                            <button
                                                key={item.id}
                                                onClick={() => scrollToId(item.id)}
                                                className={`w-full text-left transition-all duration-300 flex items-start gap-3 py-2.5 px-3 rounded-lg border-l-2 ${activeTab === item.id
                                                    ? "bg-sky-50 border-sky-500 text-sky-600 font-bold"
                                                    : "border-transparent text-slate-500 font-medium hover:bg-slate-50 hover:border-slate-200 hover:text-slate-700"
                                                    }`}
                                            >
                                                <span className={`text-[10px] font-bold tabular-nums pt-0.5 shrink-0 ${activeTab === item.id ? 'text-sky-500' : 'text-slate-300'}`}>
                                                    {String(i + 1).padStart(2, '0')}
                                                </span>
                                                <span className="line-clamp-2 leading-normal text-sm">{item.title}</span>
                                            </button>
                                        ))}
                                    </nav>
                                </div>
                    ...
                    {/* Single Document Flow Container */}
                    <main className="lg:w-2/3 xl:w-3/4 relative">
                        <div
                            ref={contentScrollRef}
                            className="bg-white rounded-sm p-6 md:p-16 border border-slate-100 shadow-[0_20px_50px_rgba(15,23,42,0.05)] max-h-[80vh] lg:max-h-[85vh] overflow-y-auto scrollbar-hide lg:scrollbar-default custom-scrollbar scroll-smooth relative"
                        >
                            {isRich ? (
                                <div className="prose prose-sky prose-lg max-w-none
                                    ...
                                    prose-p:text-slate-600 prose-p:leading-relaxed prose-p:font-medium
                                    prose-strong:text-slate-900 prose-strong:font-black
                                    prose-a:text-sky-600 prose-a:font-semibold prose-a:no-underline hover:prose-a:underline
                                    prose-ul:list-none prose-ol:list-none
                                    prose-hr:my-16 prose-hr:border-slate-100
                                ">
                                    {blocks.map((block, idx) => (
                                ...
                                <div className="space-y-14">
                                    {(data?.sections || []).map((section, idx) => (
                                        <div key={idx} className="space-y-6">
                                            {section.title && (
                                                <div className="flex items-center gap-3" id={`section-${idx}`}>
                                                    <span className="text-xs font-bold text-sky-500 tabular-nums shrink-0">{String(idx + 1).padStart(2, '0')}</span>
                                                    <h3 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight">{section.title}</h3>
                                                </div>
                                            )}
                                            {section.description && <div className="text-slate-600 text-lg leading-relaxed font-medium rich-text" dangerouslySetInnerHTML={{ __html: section.description }} />}
                                            <div className="grid md:grid-cols-2 gap-5">
                                                {section.subSections?.map((sub, sIdx) => (
                                                    <div key={sIdx} className="p-6 bg-slate-50/60 hover:bg-sky-50/40 border border-slate-100 hover:border-sky-100 rounded-sm transition-colors duration-300">
                                                        <h4 className="font-bold text-slate-800 mb-3">{sub.title}</h4>
                                                        <ul className="space-y-2.5">
                                                            {sub.bullets?.map((b, bIdx) => (
                                                                <li key={bIdx} className="text-sm text-slate-600 flex gap-2.5">
                                                                    <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-sky-400 shrink-0" /> {b}
                                                                </li>
                                                            ))}
                                                        </ul>
                                                    </div>
                        ...
                        {/* Bottom fade to signal there's more to scroll */}
                        <div className="pointer-events-none absolute bottom-0 left-0 right-0 h-16 bg-gradient-to-t from-white to-transparent rounded-b-sm" />
                    </main>
                </div>
            </div>
        </section>
            ...
            if (node.marks) {
                node.marks.forEach(mark => {
                    switch (mark.type) {
                        ...
                        case 'underline': text = <u key={i} style={{ textDecoration: 'underline' }}>{text}</u>; break;
                        case 'strike': text = <s key={i}>{text}</s>; break;
                        case 'code': text = <code key={i} className="bg-sky-50 px-1.5 py-0.5 rounded text-sm text-sky-700 font-mono">{text}</code>; break;
                        case 'subscript': text = <sub key={i}>{text}</sub>; break;
                        case 'superscript': text = <sup key={i}>{text}</sup>; break;
                        case 'link': text = <a key={i} href={mark.attrs?.href} target="_blank" rel="noopener noreferrer" className="text-sky-600 hover:underline">{text}</a>; break;
                        case 'textStyle':
                            ...
                            if (mark.attrs?.fontFamily) text = <span key={i} style={{ fontFamily: mark.attrs.fontFamily }}>{text}</span>;
                            break;
                        case 'highlight': text = <mark key={i} className="bg-sky-100 px-1 rounded text-slate-900">{text}</mark>; break;
                        default: break;
                    }
                });
            }
...
const BlockRenderer = ({ block }) => {
    ...
    switch (type) {
        ...
        case 'heading': {
            ...
            const headingClasses = "scroll-mt-32 " + (
                level === 1 ? "text-4xl" :
                    level === 2 ? "text-3xl mt-14 mb-6 pb-3 border-b border-slate-100" :
                        level === 3 ? "text-2xl mt-10 mb-4 border-l-4 border-sky-400 pl-4" :
                            level === 4 ? "text-xl mt-8 mb-3" :
                                "text-lg mt-6 mb-2"
            );
        ...
        case 'blockquote':
            return (
                <blockquote className="border-l-4 border-sky-200 bg-sky-50/40 p-6 my-10 text-slate-600 text-lg leading-relaxed rounded-r-sm not-italic">
                    {renderContent(content)}
                </blockquote>
            );
        ...
        case 'codeBlock':
            return (
                <div className="my-8 rounded-sm overflow-hidden border border-slate-200 shadow-sm font-mono text-sm bg-slate-900 group">
                    <div className="bg-slate-800 px-4 py-2 flex items-center justify-between text-slate-400 border-b border-slate-700">
                        <span className="text-[10px] uppercase tracking-widest font-bold">{attrs?.language || 'code'}</span>
                    </div>
                    <pre className="p-6 overflow-x-auto m-0 text-sky-100 leading-relaxed">
        ...
        case 'image':
            return (
                <div className="my-10 rounded-sm overflow-hidden border border-slate-100 shadow-xl group/img relative">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={attrs?.src} alt={attrs?.alt || 'Manual image'} className="w-full h-auto object-cover group-hover:scale-105 transition-transform duration-700" />
                    {attrs?.alt && <div className="absolute bottom-0 inset-x-0 p-6 bg-slate-900/60 backdrop-blur-md text-white text-[10px] font-bold uppercase tracking-[0.2em] opacity-0 group-hover/img:opacity-100 transition-opacity text-center">{attrs.alt}</div>}
                </div>
            );

        case 'table':
            return (
                <div className="my-10 overflow-x-auto rounded-sm border border-slate-100 shadow-sm">
                    <table className="w-full border-collapse bg-white m-0">
                        <tbody className="divide-y divide-slate-100">
                            {content?.map((row, ri) => (
                                <tr key={ri} className={ri === 0 ? "bg-sky-50/60" : "hover:bg-slate-50/60 transition-colors"}>
                                    {row.content?.map((cell, ci) => {
                                        const CellTag = cell.type === 'tableHeader' ? 'th' : 'td';
                                        return (
                                            <CellTag key={ci} className={`p-5 text-left text-sm border-x border-slate-50 ${ri === 0 ? 'font-bold text-slate-800' : 'font-medium text-slate-600'}`}>
                                                {renderContent(cell.content)}
                                            </CellTag>
                                        );
                                    })}
        ...
        case 'summaryBlock':
            return (
                <div className="my-10 bg-gradient-to-br from-sky-50 to-white border border-sky-100 rounded-sm p-10 relative overflow-hidden group/summary">
                    <div className="absolute top-0 right-0 w-48 h-48 bg-sky-200/20 rounded-full blur-[100px] -mr-24 -mt-24 group-hover/summary:scale-150 transition-transform duration-1000" />
                    <div className="relative z-10 flex gap-6">
                        <span className="text-6xl font-black text-sky-200 -mt-2 leading-none">"</span>
                        <div className="text-xl font-bold leading-relaxed text-slate-700 not-italic">
                            {renderContent(content)}
                        </div>
                    </div>
                </div>
        ...
        case 'blogCTA':
            return (
                <div className={`my-12 p-10 rounded-sm border flex flex-col md:flex-row items-center gap-10 ${attrs?.variant === 'minimalist' ? 'bg-slate-50 border-slate-200' : 'bg-gradient-to-br from-slate-900 via-slate-800 to-sky-900 text-white border-transparent shadow-[0_30px_70px_rgba(15,23,42,0.3)]'}`}>
                    <div className="w-28 h-28 rounded-sm bg-white/10 flex items-center justify-center p-6 relative overflow-hidden shrink-0 shadow-inner">
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img src={attrs?.image || '/images/logo.png'} alt="CTA" className="w-full h-auto object-contain relative z-10" />
                    </div>
                    <div className="flex-1 text-center md:text-left space-y-4">
                    ...
                    <a href={attrs?.buttonLink} target="_blank" rel="noopener noreferrer" className="px-10 py-4 rounded-full bg-sky-500 hover:bg-sky-400 text-white font-black text-sm transition-all transform hover:-translate-y-1 shadow-[0_15px_40px_rgba(14,165,233,0.4)] hover:shadow-[0_20px_50px_rgba(14,165,233,0.5)] active:scale-95 shrink-0">
                        {attrs?.buttonText}
                    </a>
                </div>
            );
