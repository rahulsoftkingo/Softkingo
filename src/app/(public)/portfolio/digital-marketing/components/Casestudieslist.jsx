  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4 sm:p-3 hover:shadow-lg hover:shadow-slate-200/60 transition-shadow duration-300">
      <div className="grid grid-cols-1 md:grid-cols-[120px_200px_1fr_320px] gap-5 md:gap-6 md:items-stretch">
        ...
        >
          <p
            className={`text-[12px] font-extrabold uppercase tracking-wide mb-3 ${style.statText}`}
          >
            Results Achieved
          </p>
...
function normalizeSeo(record) {
  return {
    ...
    companyLogo:
      record.companyLogo ||
      record.hero?.companyLogo ||
      record.clientOverview?.client?.logo,
    description:
      record.companyDescription ||
      record.clientOverview?.client?.description ||
      record.subtitle,
    slug: record.slug,
    industry: record?.clientOverview?.client?.industry || record?.industry || "-",
    duration: record?.clientOverview?.client?.duration || "-",
    businessImpact: record?.businessImpact?.stats ?? [],
  };
}
...
function normalizePpc(record) {
  return {
    ...
    companyLogo:
      record.companyLogo ||
      record.hero?.companyLogo ||
      record.projectOverview?.companyLogo,
    description:
      record.companyDescription ||
      record.projectOverview?.description ||
      record.hero?.description ||
      record.subtitle,
    slug: record.slug,
    industry:
      record?.industry ||
      record?.hero?.industry ||
      record?.projectOverview?.industry ||
      "-",
    duration:
      record?.hero?.duration || record?.projectOverview?.duration || "-",
    businessImpact:
      record?.achievements?.items ?? record?.performance?.metrics ?? [],
  };
}
...
function normalizeSocial(record) {
  return {
    ...
    description:
      ...
      record.subtitle,
    slug: record.slug,
    industry: record?.industry || record?.hero?.industry || "-",
...
export default function CaseStudiesList({
  ...
  const seoRecords = Array.isArray(seoData)
    ? seoData
    : Array.isArray(data)
    ? data
    : [];
  ...
  // Pehle wala original industries list logic
  const industries = [
    "All Industries",
    "Healthcare",
    "Travel & Tourism",
  ...
  // Matching logic
  const matchesIndustry = (study, selectedIndustry) => {
    if (!selectedIndustry || selectedIndustry === "All Industries") return true;
    if (!study.industry || study.industry === "-") return false;

    const currentIndustryStr = String(study.industry).toLowerCase();
    const target = selectedIndustry.toLowerCase();

    return currentIndustryStr
      .split(",")
      .map((item) => item.trim())
      .includes(target);
  };
  ...
  const visibleStudies = useMemo(() => {
    return caseStudies.filter((study) => {
      ...
      const industryMatch = matchesIndustry(study, activeIndustry);
