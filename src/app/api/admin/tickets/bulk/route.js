...
export async function PATCH(request) {
  ...
  const { ids, ownerId, status, priority, action } = body;
  ...
  if (action === 'assign') {
    ...
    await prisma.ticketComment.deleteMany({ where: { ticketId: { in: idList } } });
    await prisma.ticket.deleteMany({ where: { id: { in: idList } } });
  } else {
    return NextResponse.json(
