...
export default function CaseStudiesPage() {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      await Promise.all(
        selectedRows.map((row) =>
          fetch(`${apiPathFor(row._type)}/${row.id}`, { method: 'DELETE' })
        )
      );
      setSelectedIds([]);
      fetchItems();
    ...
    <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm hover:shadow-md hover:border-slate-300 transition-all group">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-start gap-3 min-w-0 flex-1">
          <input
            type="checkbox"
            className="h-4 w-4 mt-1 rounded border-slate-300 text-purple-600 focus:ring-purple-500 shrink-0"
            checked={selectedIds.includes(row.id)}
            onChange={() => toggleOne(row.id)}
          />
          ...
          <div className="min-w-0">
            ...
            {row.subtitle && (
              <p className="text-xs text-slate-500 truncate mt-0.5">{row.subtitle}</p>
            )}

            <div className="flex flex-wrap items-center gap-2 mt-2">
              <span
                className={`text-[10px] font-mono ${col.badgeBg} ${col.badgeText} inline-block px-1.5 py-0.5 rounded border ${col.badgeBorder} truncate max-w-[160px]`}
              >
                /{row.slug}
              </span>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-purple-50/30">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 py-4 sm:py-6 lg:py-8 space-y-4 sm:space-y-6">
        ...
        {/* Search */}
        <section className="rounded-xl sm:rounded-2xl bg-white/90 backdrop-blur-sm border border-slate-200 shadow-lg p-4 sm:p-5">
        ...
        {/* Bulk actions bar */}
        {selectedIds.length > 0 && (
          <div className="flex items-center justify-between px-4 py-2.5 sm:py-3 rounded-xl bg-gradient-to-r from-purple-600 to-purple-500 text-white shadow-lg">
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="h-6 w-6 sm:h-8 sm:w-8 rounded-full bg-white/20 flex items-center justify-center text-[10px] sm:text-xs font-bold">
                {selectedIds.length}
              </div>
